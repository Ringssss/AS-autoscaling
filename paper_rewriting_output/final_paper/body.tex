...
Suspended warm state creates a locality--placement conflict. Sticky routing preserves locality, but correlated tool completions can return many agents to one engine while another engine is idle. Least-loaded rerouting uses the idle engine, but it reconstructs every historical token there. Program-aware routers such as Agentix improve placement decisions but still choose between these two outcomes \cite{agentix}. TTL retention and source-side offload preserve or reclaim memory on the original engine \cite{continuum, tokencake}; shared KV tiers can make state available elsewhere \cite{symphony}. None of these mechanisms alone transfers the agent's continuation and future execution authority. Active-request migration systems instead move a request that is currently executing \cite{llumnix}. During AgentShift's target interval, no such request exists.
...
\item
  \textbf{Completed-prefix mobility.} AgentShift transfers and atomically installs an immutable completed prefix across identically configured SGLang engines. It preserves full prefix hits for Qwen3-8B and Qwen3-32B at tensor parallelism 1--4.
\item
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.98\textwidth]{figures/fig_workload_characterization.pdf}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.97\textwidth]{figures/fig_architecture.pdf}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.98\columnwidth]{figures/fig_context_latency.pdf}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.98\columnwidth]{figures/fig_gap_overlap.pdf}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.94\textwidth]{figures/fig_replay_policy.pdf}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.94\textwidth]{figures/fig_hotspot.pdf}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.94\textwidth]{figures/fig_elasticity.pdf}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.94\textwidth]{figures/fig_interference.pdf}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.98\columnwidth]{figures/fig_control_plane.pdf}
